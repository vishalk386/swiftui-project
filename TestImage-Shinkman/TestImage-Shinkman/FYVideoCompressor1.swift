//
//  FYVideoCompressor1.swift
//  TestImage-Shinkman
//
//  Created by Vishal Kamble on 27/09/23.
//
...
public class FYVideoCompressor1 {
    ...
    public enum VideoQuality: Equatable {
        ...
        case custom1(qualityPercentage: Int)
        ...
        var value: (fps: Float, bitrate: Int) {
            ...
            case .custom(fps: let fps, bitrate: let bitrate, _):
                return (fps, bitrate)
            case .custom1(let qualityPercentage):
                            let fps = Float(qualityPercentage) / 10.0 // Assuming 100% quality maps to 10 fps
                            let bitratePercentage = qualityPercentage
                            return (fps, bitratePercentage)
    ...
    static public let shared: FYVideoCompressor1 = FYVideoCompressor1()
    ...
    public func compressVideo(_ url: URL,
                              quality: VideoQuality = .custom1(qualityPercentage: 50),
                              outputPath: URL? = nil,
...
//            print("channels: \(2)")
    ...
    func getVideoBitrateWithQuality(_ quality: VideoQuality, originalBitrate: Float) -> Float {
        ...
        if originalBitrate < targetBitrate {
            ...
            case .custom(_, _, _):
                break
            case .custom1(qualityPercentage: let qualityPercentage):
                    targetBitrate = Float(qualityPercentage) / 100.0 * originalBitrate
                targetBitrate = max(targetBitrate, Float(Self.minimumVideoBitrate))
               }
            }
    ...
    func calculateSizeWithQuality(_ quality: VideoQuality, originalSize: CGSize) -> CGSize {
        ...
        case .custom(_, _, let scale):
            return scale
        case .custom1(qualityPercentage: let qualityPercentage):
               threshold = CGFloat(qualityPercentage) / 100.0 * max(originalWidth, originalHeight)
        }
