...
@available(iOS 14.0, *)
public struct VideoCompression {
    public struct Video {
        public struct Configuration {
            ...
            public let isSetWatermark: Bool
            ...
            public init(
                ...
                self.isSetWatermark = isSetWatermark
    ...
    public func compressVideo(
        ...
        for (index, video) in videos.enumerated() {
            ...
            if configuration.isMinBitrateCheckEnabled && bitrate <= MIN_BITRATE {
                let error = CompressionError(title: "The provided bitrate is smaller than what is needed for compression. Try setting isMinBitRateEnabled to false.")
                completion(.onFailure(index, error))
                continue
            }
            ...
            videoWriterInput.requestMediaDataWhenReady(on: processingQueue, using: { () -> Void in
                while videoWriterInput.isReadyForMoreMediaData {
                    ...
                    // Update progress based on the number of processed frames
                    frameCount += 1
                        ...
                        if videoReader?.status == .completed {
                            if audioReader != nil && !configuration.disableAudio {
                                if !(audioReader!.status == .reading) || !(audioReader!.status == .completed) {
                                    // Start writing from the audio reader
